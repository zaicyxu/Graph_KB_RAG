...
@File Name: main_rag_muti-depth_search.py
@Software: Python
@Time: Feb/2025
...
class Neo4jRAGSystem:
    ...
    def extract_keywords(self, query_text):
        ...
        We use a simple heuristic approach by keeping words with important meanings.
        """
        words = re.findall(r'\b\w+\b', query_text.lower())  # Extract words
        stopwords = {"which", "what", "is", "in", "the", "and", "satisfy", "does", "work", "on", "field"}
        keywords = [word for word in words if word not in stopwords]  # Remove stopwords
        return keywords[:3]  # Limit keywords to 3 for better accuracy

    def determine_query_depth(self, query_text):
        """
        Analyze the query and determine the optimal depth of the graph traversal.
        """
        keywords = self.extract_keywords(query_text)

        # If question cover with "sub-industry"、"sector" add the searching depth.
        deeper_keywords = {"sub-industry", "sector", "specific", "detailed", "branch", "division"}
        depth = 2

        if any(kw in deeper_keywords for kw in keywords):
            depth = 4
        return depth

    def retrieve_relevant_entities(self, query_text, top_k=3, similarity_threshold=0.4):
        """
        Retrieve entities based on extracted keywords with flexible depth for better context.
        Ensures multi-hop relationships and multiple entity types (e.g., Manufacturers, Certifications, Industries).
        """
        keywords = self.extract_keywords(query_text)
        keyword_conditions = " OR ".join([f"toLower(n.Name) CONTAINS '{kw}'" for kw in keywords])

        depth = self.determine_query_depth(query_text)  # Dynamically set the depth
    ...
    def generate_answer(self, user_question, relevant_entities):
        """
        Generate an answer using Gemini LLM, ensuring multi-hop relationships are included.
        Structured response ensures Manufacturers, Industries, and Certifications are clearly linked.
        """
        ...
        input_text = f"""You are an expert AI assistant specializing in knowledge graph analysis.
                         ...
                         If the information is not directly available, infer the most reasonable answer.
